...
def selector_str(tree):
    ...
    return {
    "Attrib" : lambda :
        (selector_str(tree.selector) if tree.selector is not None else "") +
        "[" +
        ("*|" if tree.namespace is None
              else ("" if tree.namespace == ""
                       else tree.namespace + "|")) +
        tree.attrib +
        tree.operator +
        _str_unicode(tree.value) +
        "]",
    "CombinedSelector" : lambda :
        selector_str(tree.selector) +
        " " + tree.combinator + " " +
        selector_str(tree.subselector),
    "Class" : lambda :
        (selector_str(tree.selector) if tree.selector is not None else "") +
        "." + tree.class_name,
    "Element" : lambda :
        (tree.namespace + "|" if tree.namespace is not None else "") +
        (tree.element if tree.element is not None else "*"),
    "Function" : lambda :
        (selector_str(tree.selector) if tree.selector is not None else "") +
        ":" + tree.name +
...
class CSSRule:
    ...
    def __str__(self):
        return (",".join([selector_str(s.parsed_tree) for s in self.__selectors]) +
                "{" +
                ";".join([str(p) + ": " + str(v)
...
class CSSFile:
    ...
    def __selector_str(self, selector):
        ...
        :returns:
            A string representation of selector
        """
        return selector_str(selector.parsed_tree)
    ...
    def __build_data_structures(self, multiprop):
        ...
        :returns:
            ...
            rules is set of pairs (S, P) where S is set of cssselect parsed_tree
            selectors and props is a set of pairs (p, v) of strings property and
            value
        """
        ...
        for rule in islice(self.stylesheet.rules,
            ...
            decls = [ (decl.name,
                       self.__normalise_css_value(decl.value.as_css()))
                      for decl in rule.declarations ]

            for sel in self.__parse_selector(rule.selector):
                sels.add(sel)
                for decl in declarations:
                    v = self.__normalise_css_value(decl.value.as_css())
                    specificity = (decl.priority is not None,
                                   sel.parsed_tree.specificity())
                    tup = (sel.parsed_tree, v)
